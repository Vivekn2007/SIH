...
function renderRadar(canvas: HTMLCanvasElement, emitters: Emitter[], smartBand: number, filter: Filter, phase: number) {
  ...
  for (let i=1;i<=5;i++) { ctx.beginPath();ctx.arc(0,0,r*i/5,0,Math.PI*2);ctx.stroke() }
  ...
  emitters.forEach((e, index) => { const distance=r*(.22 + ((index*17+e.band_id*7)%65)/100); const angle=(e.aoa_deg-90)*Math.PI/180; const x=Math.cos(angle)*distance,y=Math.sin(angle)*distance; const threat=e.threat_level==='High'; const locked=(filter !== 'open' && e.band_id===smartBand); ctx.save();ctx.translate(x,y)
...
export function RadarPpi({ telemetry, filter }: { telemetry: Telemetry | null; filter: Filter }) {
  const ref=useRef<HTMLCanvasElement>(null), data=useRef({ emitters: [] as Emitter[], band: -1, filter }); data.current={emitters:telemetry?.ground_truth.active_emitters ?? [],band:telemetry?.smart_scan.dwelling_band ?? -1,filter}
  useEffect(()=>{let frame=0;const start=performance.now();const draw=(now:number)=>{if(ref.current)renderRadar(ref.current,data.current.emitters,data.current.band,data.current.filter,(now-start)/1100);frame=requestAnimationFrame(draw)};frame=requestAnimationFrame(draw);return()=>cancelAnimationFrame(frame)},[])
  const es=telemetry?.ground_truth.active_emitters ?? []; const threats=es.filter(e=>e.threat_level==='High').length;const locked=es.filter(e=>e.band_id===telemetry?.smart_scan.dwelling_band).length
  return <Panel title="RADAR PPI — 360° SITUATIONAL AWARENESS" className="radar-panel"><div className="radar-body"><div className="radar-info"><p>RANGE MAX: <b>300 km</b></p><p>RINGS: <b>6 (50 km)</b></p><p>MODE: <b>ES SURVEY</b></p><p>ANTENNA: <b>4-AXIS</b></p><p>LAT: <b>25.3147° N</b></p><p>LON: <b>121.4737° E</b></p></div><canvas className="radar-canvas" ref={ref}/><div className="radar-readout"><p>TRACKS: <b>{es.length}</b></p><p>LOCKED: <b>{locked}</b></p><p>THREATS: <b className="red">{threats}</b></p><hr/><p>JAMMING: <b>OFF</b></p><p>GEOFENCE: <b className="green">ON</b></p><p>AUTO-CLASS: <b className="green">ON</b></p><p>TRACK FUSION: <b className="green">ON</b></p></div></div><div className="legend"><span className="dot cyan"/>Friendly / Unknown <span className="triangle"/>Hostile / Threat <span className="square"/>Unclassified <span className="lock-ring"/>Locked Target</div></Panel>
}
